...
import { vadd, vdot, Vec2, vlen, vmul, vnorm, vsub } from './vector';
...
export class Block {
  ...
  public instant(tU: number, dt: number = 0, ds: number = 0): Instant {
    ...
    const s = Math.max(
      0,
      Math.min(this.distance, this.vInitial * t + (a * t * t) / 2)
    );
 ...
 */
function constantAccelerationPlan(
  points: Vec2[],
  ...
  const segments = dedupedPoints
    .slice(1)
    .map((a, i) => new Segment(dedupedPoints[i], a));
  ...
  segments.slice(1).forEach((seg2, i) => {
    ...
    seg2.maxEntryVelocity = cornerVelocity(
      seg1,
      seg2,
      vMax,
      accel,
...
export function plan(paths: Vec2[][], profile: ToolingProfile): Plan {
  ...
  paths.forEach((p, i) => {
    ...
    const penUpPos =
      i === paths.length - 1
        ? Device.Axidraw.penPctToPos(penMaxUpPos)
        : profile.penUpPos;
    motions.push(
