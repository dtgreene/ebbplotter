...
export function getMotionPlan(path, position = { x: 0, y: 0 }) {
  ...
  if (motionSegments.length > 0) {
    ...
    if (penUpCommands.length > 0) {
      result = result.concat(penUpCommands);
    }
    // Lower the pen
    comresultmands.push(COMMANDS.setPenState(PEN_STATES.down), SERVO_DURATION);
    ...
    // Raise the pen
    result.push('SP,0', SERVO_DURATION);
  } else {
...
function getMotionCommands(motionSegment, maxSpeed, entrySpeed, exitSpeed) {
  ...
  if (segmentLength < MIN_MOTION_PROFILE_DISTANCE * 2) {
    return [
      COMMANDS.STEPPER_MOVE_ACCELERATED(x1, y1, x2, y2, entrySpeed, exitSpeed),
    ];
  }
  ...
  if (profileFinal.length > 0) {
    ...
    for (let i = 0; i < profileFinal.length; i += 2) {
      ...
      result.push(
        COMMANDS.STEPPER_MOVE_ACCELERATED(
          lastX,
          lastY,
          pointX,
          pointY,
...
function getMotionSegment(x1, y1, x2, y2) {
  ...
  if (length > 0) {
    ...
    maxSpeedDelta = Math.sqrt(2 * MOTION_ACCELERATION * length);
  }
