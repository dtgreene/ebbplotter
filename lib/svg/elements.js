...
export function getPathPoints(data, transform) {
  ...
  // Convert smooth curves to generic
  // Convert all commands from relative to absolute
  // Reduce the command range from MCLHVQZ to MLCZ
  const reducedPath = SVGPath.from(data)
    .transform(transform)
    .unarc()
    .unshort()
    .abs()
  ...
  reducedPath.iterate((segment, _, currentX, currentY) => {
    const command = segment[0];
